window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *
		 * 0.1.5-rc.1 契约复核（2026-09-10，逐包源码比对）：
		 * - `dsh-client-connection` 客户端仍以 `handle.isLoopback` 暴露判定（可写），
		 *   未冻结 → 改 connection.isLoopback 的做法在 0.1.5 依旧有效。
		 * - 0.1.2 起 settings 的持久化入口换成 `ctx.remote.$host.isLoopback`
		 *   （`dsh-api-gateway` 的 ClientRemoteService），其 getter 每次现读
		 *   `connection.isLoopback` —— 因此改 connection 就能让 `$host` 反映出来。
		 * - 但 `$host` 会把结果缓存进 `hostFacts`（缓存键只有 `home`），若它在我们
		 *   修正之前已被读取，缓存会一直停在 `isLoopback: false`。0.1.5 的
		 *   `settingsScope` 与 settings-general 的「打开配置文件」入口都在 activation
		 *   期读 `$host.isLoopback`，故这里额外把 `remote.hostFacts` 的缓存一并修正。
		 */
